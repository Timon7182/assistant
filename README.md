...
- Молчаливый режим (галочка в шапке дашборда или `SILENT=1`): с людьми не разговаривает, незнакомые лица сохраняет как «Гость N» с кадром и временем, имя задаётся в дашборде кнопкой «Имя». Вызов по слову/кнопке работает и в этом режиме.
...
В консоли клиента можно печатать текст вместо речи (отладка), пустая строка = вызов. `--camera -1` без камеры (только голос/кнопка). Звуковая обратная связь: один сигнал = услышал слово-активатор/кнопку, два сигнала = фразу записал и обрабатывает, потом голосовой ответ (`--no-beep` отключает), `--mic N --speaker N` выбор устройств (`python -m sounddevice`). Модель для слова-активатора (~45 МБ) скачивается сама при первом запуске в `edge/models/`. Короткие слова-активаторы (одно слово в 2 слога) чаще срабатывают ложно, лучше два слова ("эй помощник").
